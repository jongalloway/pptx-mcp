...
### Decision: Table Tool Design (Issue #36)
...
Table insert and update tools follow MVP scope:
- **Insert**: Headers + rows arrays, auto-pad short rows, no style/color params
- **Update**: Cell text replacement only, no structural changes (add/remove rows/cols)
- **Locate**: By name (case-insensitive) or zero-based index among tables on slide
- **Preserve**: TableCellProperties kept on update; TextBody rebuilt with clean structure

#### Rationale

- Cell-text-only updates avoid TableGrid recalculation complexity
- Name + index targeting matches existing shape resolution pattern from `pptx_update_slide_data`
- Error messages list available tables so agents can self-correct
- Structural mutations (add/remove rows/cols) deferred — requires coordinated TableGrid + cell count changes

#### Impact

- Phase 3 table tools (#36) complete with this scope
- Future work: row/column append, style parameters, merge cells

---

### Decision: Table Tests Validation Pattern
...
For OpenXML validation in table tool tests (and all future tests), use the **baseline comparison pattern** — capture validation errors before the operation, then assert the count hasn't increased afterward. Do NOT use `Assert.Empty(errors)` because TestPptxHelper fixtures produce pre-existing SlideMaster validation errors that are harmless.
...
TestPptxHelper's minimal presentation structure triggers MinInclusive constraint warnings on SlideMasterPart. These are benign (PowerPoint opens the files fine) but cause `Assert.Empty` to fail. The baseline pattern catches real regressions while tolerating known fixture limitations.

#### Scope

Applies to all test files in `tests/PptxMcp.Tests/`.

---

### Decision: Cheritto Template Slide Placeholder Targeting
...
For template-aware authoring, placeholder inputs should use semantic keys in `Type` or `Type:Index` form rather than slide-local shape names.
...
Template-backed authoring needs stable targeting across layouts, duplicated slides, and placeholder clones where shape names are often generic or regenerated. Semantic placeholder identity maps better to OpenXML placeholder metadata (`type` + `idx`) and matches the MarpToPptx prior-art pattern Nate surfaced.

#### Implementation Impact

- `pptx_add_slide_from_layout` and `pptx_duplicate_slide` both accept semantic placeholder dictionaries.
- Title and centered-title placeholders are normalized to the same `Title` semantic bucket.
- Text writes are only allowed against text-capable placeholders; picture placeholders resolve semantically but reject text overrides with a clear error.
