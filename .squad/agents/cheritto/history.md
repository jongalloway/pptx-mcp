...
### Image replacement tool (2026-03-17)
- `pptx_replace_image` replaces images in existing picture shapes by name (case-insensitive) or zero-based index among `P.Picture` elements on the slide.
- Geometry inheritance from layout is preserved: the service only replaces the `Blip` relationship, never touches `ShapeProperties` or `Transform2D`.
- The service creates a new `ImagePart`, updates the `Blip.Embed` reference, and cleans up unused old image parts by checking if any other element still references the old relationship ID.
- Alt text is set via `NonVisualDrawingProperties.Description` on the picture shape.
- Supported formats: PNG (image/png), JPEG (image/jpeg), SVG (image/svg+xml). Unsupported formats return a structured error before opening the file.
- Pattern follows `UpdateSlideData`: thin tool → service method → internal target resolution (`GetPictureTargets` / `ResolvePictureTarget`) mirroring the text shape targeting approach.
- `GetImageContentTypeString` returns content type strings for the 3 supported formats; `GetImagePartType` (pre-existing) maps to `ImagePartType` enum.
- Key files: `src/PptxMcp/Models/ImageReplaceResult.cs`, `PresentationService.ReplaceImage()`, `PptxTools.pptx_replace_image()`
- **Testing:** 26 new tests (21 service + 5 tool), 205/205 passing
- **Deliverable:** PR #47 (implementation + tests, build clean)
